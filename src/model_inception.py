...
─────────────────────────────────────────────────────────────────────────────
Purpose:
    Defines the InceptionV3 transfer learning model for the Lung Cancer
    Detection project.

    Preprocessing note:
        Preprocessing is handled by the data generator via the
        preprocessing_function parameter (inception_v3.preprocess_input),
...
def build_inception(input_shape=(224, 224, 3), num_classes=3):
    """
    Builds InceptionV3 with a custom classification head.
    ...
    print(f"  InceptionV3 built — Stage 1 (base frozen)")
...
def unfreeze_inception(model, fine_tune_layers=30):
    """
    Unfreezes the last N layers of InceptionV3 for Stage 2 fine-tuning.

    Args:
        model            (tf.keras.Model): Model from build_inception().
    ...
    trainable = sum([len(l.trainable_weights) for l in model.layers])
    print(f"  InceptionV3 — Stage 2 (last {fine_tune_layers} layers unfrozen)")
    print(f"  Trainable weight tensors : {trainable}")
    return model